...
class GraspRectangles:
    ...
    @classmethod
    def load_from_cropdata_file(cls, fname, scale=1.0, normalize=True):
        """
        Load grasp rectangles from a crop dataset dataset file.
        :param fname: Path to file.
        :param scale: Scale to apply (e.g. if resizing images)
        :return: GraspRectangles()
        """
        ...
        with open(fname) as f:
            ...
            for l in f:
                if l == 'None':
                    ...
                    # print('none!')
                    for i in range(10):
                        grs.append([0, 0, norm_q])
                else:
                    x, y, theta, w, q = [float(v) for v in l[:-1].split(';')]
                    # theta, w, q = [float(v) for v in l[:-1].split(';')]
                    
                    #if theta ==0 or w ==0:
                     #   print(99999999999999999, theta,w,q, fname)
                    if normalize:
                        #norm_theta = round((theta - (-1)) / (360 - (-1)),4)
                        #norm_w = round((w - 8) / (47 - 8),4)
                        #norm_q = round((q - 20) / (50 - 20),4)
                        # print('label origin', theta, w)
                        norm_theta = (theta - (-1)) / (360 - (-1))
                        norm_w = (w - 8) / (47 - 8)
                        norm_q = (q - 20) / (50 - 20)
                        # print(33, type(norm_theta))
                        # print('label after', norm_theta, norm_w)
                        grs.append([norm_theta, norm_w, norm_q])
                        #if norm_theta == 0 or norm_w == 0:
                         #   print(888888, norm_theta , norm_w)
                    else:
                        grs.append([theta, w, q])
                
            if len(grs) != 10:
                # print('not 10!' , len(grs))
                for i in range(10 - len(grs)):
                    grs.append([0,0,0])

        if grs[0][0] == 0 and grs[0][1] == 0 and grs[0][2]==1:
            print('ori data:', theta,w,q)  

        return np.array(grs, dtype=np.float64)

    @classmethod
    def load_parallel_crop_file(cls, fname, scale=1.0, normalize=True, angle_st= (-1), angle_ed=361):
        """
        Load grasp rectangles from a crop dataset dataset file.
        :param fname: Path to file.
        :param scale: Scale to apply (e.g. if resizing images)
        ...
        with open(fname) as f:
            ...
            for l in f:
                ...
                if normalize:
                    ...
                    norm_theta = (theta - (angle_st)) /(angle_ed - angle_st) 
                    norm_w = (w - 8) / (47 - 8)
                    norm_q = (q - 20) / (50 - 20)
        ...
        return np.array(grs, dtype=np.float32)
...
class Grasp:
    ...
    def __init__(self, center, angle, length=60, width=30):
        self.center = center
        self.angle_ori = angle # Positive angle means rotate anti-clockwise from horizontal.
...
def detect_grasps_parallel( angle, width, no_grasps=1):
    """
    Detect grasps in a network output.
    :param angle: Angle value of output
    :param width_img: (optional) Width image network output
    :param no_grasps: Max number of grasps to return
    :return: list of Grasps
    """
